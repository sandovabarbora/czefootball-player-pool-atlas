...
def test_export_ages_walks_first_top9_season():
    t = pd.DataFrame({
    ...
    ages = _export_ages(t, ["GER-Bundesliga", "ENG-Premier League"], "CZE", "2025-2026")
    assert ages == [23.0]  # first top-9 season 2023-2024, born 2000 -> 23


def test_gk_first_top9_ages_pairs_name_with_age():
    t = pd.DataFrame({
        "league": ["GER-Bundesliga", "ENG-Premier League"],
        "season": ["2024-2025", "2025-2026"],
        "team": ["Mainz", "Arsenal"],
        "player": ["Jan Novak", "Jan Novak"], "player_key": ["jan novak|2000"] * 2,
        "nation": ["CZE"] * 2, "born": [2000, 2000], "min": [1500, 1800],
    })
    rows = gk_first_top9_ages(t, ["GER-Bundesliga", "ENG-Premier League"], "CZE", "2025-2026")
    assert rows == [{"player_key": "jan novak|2000", "player": "Jan Novak",
                     "first_age": 24.0, "first_season": "2024-2025"}]
...
def test_club_tier_joins_goals_scored_percentile():
    ...
    out = club_tier(keepers, players_all, "CZE", HEADLINE, "2025-2026")
    assert len(out) == 1  # only "CZE Keeper" is home nation + headline league
    row = out.iloc[0]
...
def test_production_table_adds_quality_adjusted_ga90():
    ...
    assert abs(row.ga90_q - row.ga90_shrunk * 1.0) < 1e-9
...
def test_build_goalkeepers_json_shape():
    ...
    players_all = pd.DataFrame({
        "league": ["ENG-Premier League"] * 2, "season": ["2025-2026"] * 2,
        "team": ["Team A", "Team B"], "player": ["Out Field", "Out Field 2"],
        "player_key": ["out field|1999", "out field 2|1998"], "nation": ["CZE", "CZE"],
        "pos": ["FW", "MF"], "born": [1999, 1998], "gls": [40, 60], "min": [1800, 1500],
    })
    peers_meta = {"CZE": {"name": "Czechia", "population_m": 10.9}, "SVK": {"name": "Slovakia", "population_m": 5.4}}
    league_quality = {"multipliers": {"ENG-Premier League": 1.0, "GER-Bundesliga": 0.788, "CZE-First League": 0.434}}
    nt = pd.DataFrame({"player_norm": [], "born": [], "event": []})
    out = build_goalkeepers(
        keepers, players_all, peers_meta, HEADLINE, "CZE", "2025-2026", "2025-2026",
