...
(function () {
  ...
  function renderEras() {
    ...
    box.innerHTML =
      ...
      `<p class="ax-note">Bars: distinct home-nation players with at least one appearance in the Premier League, Serie A, La Liga, Bundesliga or Ligue 1 that season (FBref season tables; the English top flight from 92/93, Ligue 1 from 95/96 — seasons with fewer than five leagues are hatched). Line: their median age. Acid marks: debutants — a first Big-5 season of 450+ minutes — a proxy for when the exports arrive; the first season of the history has none by construction. The pool's own leagues (the home league, its peers) are covered only from ${short(SEASONS[0])}, so this is the top of the ladder over time, not the whole pool.</p>` +
  ...
  function erasChart(box, S) {
    ...
    svg.append('g').selectAll('rect').data(S).join('rect')
      ...
      .attr('fill', (s) => (s.season === eraSeason ? C.acid : C.hot)).attr('opacity', (s) => (s.season === eraSeason ? 1 : s.leagues && s.leagues < 5 ? 0.45 : 0.85))
