...
EN: dict[str, str] = {
    ...
    "toc.q10": "What changed in a year",
    ...
    "hero.finding": "Per head, {nation} ranks {rank} of {n} peer countries for players in Europe\u2019s strongest leagues.",
    "hero.finding.lead": "The reasons below are measured, not guessed:",
    "hero.finding.youth": "{per_club} regular under-21 starters per club at home against {best_per_club} in {best}",
    "hero.finding.move": "a first move abroad at {age}",
    "hero.finding.break": "a layer at the top that stepped down in {season}",
    "hero.path": "Three minutes: this number, the five rungs below it, and the one-page brief. Everything else is the evidence.",
    ...
    "why_funnel.stage1.note": "They are not being used as late substitutes: {starts} % of the league\u2019s starts went to them against {minutes} % of its minutes, and when one of them started he was on the pitch for {mn} minutes against the league\u2019s own {league_mn}. The difference between these countries is how many young players are picked at all, not how they are used once picked.",
    "why_funnel.stage1.note_early": "The shortage is one of selection, not of late cameos: {starts} % of the league\u2019s starts went to them against {minutes} % of its minutes, so they are picked to start about as often as they are played at all. They are, though, taken off early \u2014 {mn} minutes in a start against the league\u2019s own {league_mn}.",
    ...
    "why_funnel.take_one_thing.note": "That is one decomposition over {n} countries \u2014 a description of the gap, not a weight to plan by.",
    ...
    "why_funnel.stage3.metric1": "Age the first time a player has real playing time in a foreign league, over the players abroad today",
    ...
    "slide.3.uncertain": "How sure is the link to the top? Across {n} countries, ten points more under-21 share go with {beta} more top-{topn} players per million, but the interval ({lo} to {hi}) includes zero; within countries, from one season to the next, the estimate is {within} ({wlo} to {whi}) \u2014 nothing. The share is a fact; its weight in the gap is not settled.",
    ...
    "slide.4b.a": "Arrive in the top-{topn} at 21 and you produce <strong>{diff}</strong> more league-adjusted G+A per 90 over your first two seasons than arriving at 24. {Adj} exports arrive at a median age of {age_home}.",
    "slide.4b.a_zero": "<strong>No.</strong> Players who reach the top-{topn} at 21 and at 24 produce the same league-adjusted G+A per 90 over their first two seasons (difference {diff}, an interval that includes zero, n = {n}). {Adj} exports arrive at a median age of {age_home}.",
    ...
    "slide.5.caption": "How to read it: one row per country, sorted; the dot is the median share of his club\u2019s minutes that a country\u2019s exported player keeps, the thin line the range across its exports. {Adj} exports are the acid row. Further right means exports who play, not sit.",
    ...
    "slide.7.aq": "In numbers: {adj} players with ≥ {min} minutes in the Big-5 leagues each season since {history_start}, with the rise and the fall each dated by a change-point model.",
    ...
    "slide.7.a_detail_rise": " Rise posterior {rise_prob}; ×{rise_delta}.",
    "slide.7.how": "FBref Big-5 player tables {first_season} → {last_season}; peers on the same rule; the names are the most-minutes {adj} players of each peak season, goalkeepers included — a lineup of presence, not a quality ranking: {golden}. The break dates come from a Bayesian local-level model with two ordered change points, one for the rise and one for the fall {cite_cp}; detail in <a href=\"#series-model\">§ Methodology</a>.",
    ...
    "slide.8.caption": "How to read it: six measures, each rescaled so 0 is the worst of the three countries and 1 the best, one line per country. A line that stays high is a pathway that is open at every stage; where the acid line dips is where {nation} loses ground.",
    ...
    "slide.10.q": "What changed since last season?",
    ...
    "slide.10.did": "We took each player\u2019s main league last season and this season, sorted the leagues into the four rungs the report uses everywhere \u2014 home league, other covered league, stepping stone, top nine \u2014 and counted who climbed, who came down, who appeared and who is no longer in any league we can see.",
    "slide.10.a": "<strong>{up} climbed a rung, {down} came down.</strong> The stepping-stone leagues hold {ss_curr} of the pool, from {ss_prev}; the top nine hold {top_curr}, from {top_prev}.",
    "slide.10.minutes": "{prev} → {curr} minutes",
    "slide.10.tiers_note": "Players of the pool on each rung, {previous} → {metrics}, with the minutes they played there; {n_prev} qualifying players last season, {n_curr} this season.",
    "slide.10.move_up": "Climbed a rung",
    "slide.10.move_down": "Came down a rung",
    ...
    "pred.backtest": "The model behind this number, refitted at {n} past origins, missed by {mae_model} players on average against {mae_naive} for \u201csame as last year\u201d \u2014 it has not yet beaten the baseline it is scored against.",
    ...
    "slide.7.did": "We counted, season by season, how many home-nation players had real playing time in Europe's five biggest leagues, then looked for the two seasons where that count changed level for good — once up, once down — and stayed there.",
    ...
    "ch1.atlas.alt": "Two-panel atlas of {group} {season} in PCA projection. Left panel: style map without league multipliers; right panel: quality-adjusted map. Grey points are the whole corpus of {corpus} players; coloured points are the {czech} {adj}-eligible players by cluster; bright rings mark the {nt} with a national-team call-up {nt_years}.",
    "ch1.atlas.caption": "Atlas of {group} {season} in both projections: {czech} {adj}-eligible players in colour against a corpus of {corpus}. Bright rings mark the national-team pool (call-up {nt_years}, {nt} players).",
    ...
    "ch4.series.design": "The series is modelled as a local level in state space {cite_ss}: the log of the season count follows a Gaussian random walk (σ ~ HalfNormal(0.2)), plus two ordered step changes δ₁, δ₂ in the level at unknown seasons τ₁ < τ₂ — on the series back to 1990/91 one step is misspecified, since the count rises through the 1990s and falls after the 2000s plateau, and a single step lands on whichever change buys more likelihood. NUTS only samples continuous parameters, so the τ pair is not sampled directly — every ordered pair of candidate seasons at least {margin} seasons from either end and from each other is marginalised out of the model with a single log-sum-exp potential, and each break's own posterior is recovered afterwards from the continuous draws, the standard move for a marginalised discrete parameter {cite_marg}; the changepoint idea itself is due to {cite_cp}, applied here to a batch, two-break setting. The report's \"break\" is the step that lowers the level; the other is the rise. A rolling-origin backtest {cite_ro} refits the same local level without the step at each of {n_backtest} origins, forecasting one season ahead and scoring against the naive \"same as last season\" baseline — the honest forecaster's read, since no real origin knows in advance which side of a break it sits on. The one forecast below, from the same change-point-free model fitted on the full series, is a demonstration of the method on a count of players, not a statement about any player.",
    "ch4.series.fig_alt": "Line chart of the {nation} series with the change-point model's fitted level in a pale band, a vertical rule marking the most probable break season with its posterior probability, and the one-season forecast with its 90 % interval at the right edge, past a dashed divider.",
    "ch4.series.caption": "How to read it: the acid line is the observed count, the pale band the model\u2019s fitted level; the dashed rules are the most probable rise and fall seasons with their posterior probabilities; past the divider on the right, the one-season forecast with its 90 % interval.",
    ...
    "ch4.export.fig_alt": "Line chart: the fitted age-at-export curve with its 90 % band, home-nation exports as acid points against every other peer export in grey, and a rug of every export's age along the axis.",
