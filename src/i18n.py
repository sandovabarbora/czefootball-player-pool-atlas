...
EN: dict[str, str] = {
    ...
    "slide.3.panel": " Across the {n_countries} countries and two seasons, 10 points more U21 share go with {beta} more top-{topn} players per million ({lo}–{hi}).",
    ...
    "slide.8b.q": "Where does it work: goalkeepers?",
    "slide.8b.a": "{n_gk} {adj} goalkeepers play ≥ {min} minutes in the top-{topn} leagues ({pm} per million, rank <strong>{rank} of {n}</strong>); they first appeared there at a median age of {gk_age}, against {out_age} for outfield exports.",
    ...
    "slide.8c.how": "Ridge-regression linear split, Blinder-Oaxaca-style ({cite_oaxaca}; {cite_blinder}), fit on the {n} peer countries with data on all three channels; bootstrap 90 % intervals, {n_boot} resamples; a decomposition of a correlation, not a causal accounting.",
    ...
    "ch4.panel.design": "For each of the {n_countries} peer countries and two seasons ({seasons}), U21 share of domestic-league minutes (x) against top-9 players per million (y); n = {n}. A Bayesian regression, y ~ Normal(α + β·x + u_country, σ), with a country random intercept for partial pooling ({chains} chains × {draws} draws {cite_pymc}) — with only two seasons per country the intercept is weakly identified, so most of the cross-country pattern is available to it rather than to β, leaving β's own interval wide {cite_bda}. Compared against a plain pooled least-squares slope (numpy polyfit, ignoring country structure entirely) with a percentile-bootstrap 90 % interval ({n_boot} resamples) — statsmodels is not a dependency here.",
    "ch4.panel.fig_alt": "Scatter of the panel with the population-level fitted line and its 90 % band.",
    "ch4.panel.bootstrap": "β = {beta} per 10 percentage points of U21 share (90 % HDI {lo}–{hi}), R² = {r2} (posterior-median fit, fixed effect + country intercept); the pooled OLS slope is {ols} ({ols_lo}–{ols_hi}). n = {n}; the interval is wide because the panel is small.",
    "ch4.panel.diagnostics": "R-hat ≤ {rhat}, minimum bulk ESS {ess}, {div} divergent transitions; posterior-median country-intercept scale σ_country = {sigma_country}.",
    "ch4.panel.note": "Descriptive only: a season's own U21 share and that same season's own per-capita count are shown together because they are the numbers on hand, not because one is claimed to produce the other.",
    ...
    "ch4.gap.design": "Ridge regression (α = {alpha}, standardised channels, converted back to original units) of top-9 players per million on three channels — U21 share of domestic minutes, domestic league strength (<a href=\"#league-strength\">M2's m_L</a> where the country's top flight is in that model's fitted set, else the UEFA-coefficient multiplier — the panel table below states which for each row) and median export age (recent entrants, or the all-time median for a country with none recently) — over the {n} peer countries with data on all three channels, metrics season. For one contrast country at a time, the fitted model's own prediction splits the gap exactly into one term per channel — the linear-model form of the Blinder-Oaxaca wage decomposition ({cite_oaxaca}; {cite_blinder}), applied here to a per-capita player count. Three linear channels means this split is Shapley-equivalent: the Shapley value of an additive term in a linear model equals its own coefficient's contribution, so no permutation machinery is implemented. The residual is whatever the three channels do not carry.",
    ...
    "ch4.validation.m3": "The break and forecast (M4): the break dates to {season} ({prob} posterior) for {nation}, a ×{delta} level change; the plain local level beats the naive baseline on MAE ({mae_model} vs {mae_naive}) with {coverage} of the 90 % intervals covering the observed value over {n} rolling-origin backtests.",
    "ch4.validation.m4": "The youth-minutes panel (M3): across {n_countries} countries and two seasons (n = {n}), the country-intercept model's slope is {beta} per 10 percentage points of U21 share ({lo}–{hi}), R² = {r2}; a plain pooled OLS slope is {ols} ({ols_lo}–{ols_hi}) — the interval is wide because the panel is small.",
