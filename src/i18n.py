...
EN: dict[str, str] = {
    ...
    "hero.meta.nt": "NT call-up {nt_years}",
    ...
    "finding.1": "Per-capita density places the Czech pool {rank} of {n} countries in the peer set at {pm} per million; {top} leads at {top_pm} per million, {ratio} times denser.*",
    ...
    "finding.4": "Czech exports keep {cze} % of their club's minutes — rank {rank} of {n} countries in the peer set; the furthest peer, {extreme}, sits at {extreme_value} %.*",
    ...
    "ch3.cards.rules": "Why these cards: one card per position group per rule, applied in this order — (a) highest quality-adjusted npG+A per 90, (b) youngest national-team call-up, (c) most top-9 minutes among the {event} squad, (d) most top-9 minutes, (e) most domestic minutes under 23 without a top-9 season. A player already chosen by an earlier rule falls through to the next name, so a later row can show the second name by its measure.",
    ...
    "pi.th.nt": "NT",
    "pi.nt_yes": "NT",
    "pi.note": "* {season} season, at least {min} minutes; the club is the one with the most minutes that season. npG+A/90 q = quality-adjusted non-penalty goals + assists per 90. NT = national-team call-up {nt_years}.",
